    ...
    <section id="features" className="py-16 md:py-24 bg-lingoowl-light">
      <div className="max-w-7xl mx-auto px-4 md:px-10">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-[#10162f] max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          {features.map((feature, index) => (
            <div key={index} className="flex flex-col items-center text-center">
              <div className="bg-[#FFFAE5] p-4 rounded-full mb-6 shadow-sm border-2 border-[#10162f]">
                <feature.icon className="h-8 w-8 text-[#10162f]" />
              </div>
              <h3 className="text-xl font-bold mb-4">{feature.title}</h3>
              <p className="text-[#10162f] mb-6">{feature.description}</p>
              <Button variant="outline" className="border-lingoowl-purple text-[#10162f] hover:bg-lingoowl-light">
                {feature.action}
              </Button>
            </div>
          ))}
